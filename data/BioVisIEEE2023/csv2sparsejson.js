    ...
    fs.createReadStream(file)
        ...
        .on('data', (row) => {
            if (isNaN(row[0]) || isNaN(row[1]) || isNaN(row[2])) {
                ...
                console.error('row[0] = ', row[0], 'row[1] = ', row[1], 'row[2] = ', row[2]);
                return;
            }
        ...
        .on('end', () => {
            ...
            console.log(`File ${file} is being processed`);

            for (let i = 0; i < rows.length; i++) {
                sparseMatrix.set([rows[i], cols[i]], values[i]);
		if(i%10000 === 0) {
            		console.log(`Processing: ${i}, ${rows[i]}, ${cols[i]}`);
		}
            }

            console.log(`File ${file} has been processed and is now being saved...`);
