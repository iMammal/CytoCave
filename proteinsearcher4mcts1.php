...
// echo "Searchinng for: " . $searchGene;
echo "Searching for: " . json_encode($searchGeneArray);
...
// load LookupTable_MCTS4PSM5.csv file into memory

// Read CSV file
$csvFile = 'data/LookupTable_MCTS4PSM6.csv';
if (!file_exists($csvFile) || !is_readable($csvFile)) {
    echo 'CSV file not found or not readable.';
    exit;
}
...
//     Insert into database
   ...
   $stmt->bindValue(':label', $genecounter, PDO::PARAM_STR);
    $stmt->bindValue(':complexid', $searchGeneList[0][0] . '_' . $row['Score_Max'], PDO::PARAM_STR);
    $stmt->bindValue(':region_name', $row['region_name'], PDO::PARAM_STR);
    $stmt->bindValue(':uniprotAcc', $row['uniprotAccs'], PDO::PARAM_STR);
    $stmt->bindValue(':confidence', $row['Score_Max'], PDO::PARAM_STR);
    $stmt->bindValue(':hemisphere', $row['hemisphere'], PDO::PARAM_STR);
    $result = $stmt->execute();

    if (!$result) {
        echo "Error inserting data into tempMetadata: " . implode(", ", $pdo->errorInfo());
        continue;
    }

    // Add to array
    //         'complexid' => $complexId . '_' . $complexcounter,
    $tempMetadata[$genecounter] = array(
        'label' => $genecounter,
        'complexid' => $searchGeneList[0][0] . '_' . $row['Score_Max'],
        'region_name' => $row['region_name'],
        'uniprotAccs' => $row['uniprotAccs'],  //[$genecounter % count($uniprotAccs)], // Example logic for assigning uniprotAcc
        'confidence' => $row['Score_Max'],
        'hemisphere' => $row['hemisphere']
    );

    $genecounter++;
    //$complexcounter++;
    if ($row['hemisphere'] == 'right' && !$firstRightLabel) {
        $firstRightLabel = $genecounter;
    }
}

// Print the resulting array
echo "tempMetadata:";
print_r($tempMetadata);


// echo "LookupTable: " . json_encode($lookupTable);
...
foreach ( $searchGeneList[0] as $assy) {
  ...
  $genenamesInComplexes[$complexIdAssy] = array();

  // split the genenames by space
  $genenamesRaw = explode(" ", $row['genenames']);

  echo "GenenamesRaw: " . json_encode($genenamesRaw) . "<br>";
  ...
  foreach ($genenames as $genename) {
    ...
    array_push($genesInComplexes[$complexIdAssy], array('label' => $genecounter, 'region_name' => $genename));
    ...
    $stmt->bindValue(':label', $genecounter+$firstRightLabel-1, PDO::PARAM_STR);
    ...
    array_push($tempTopology,array(
      'label' => $firstRightLabel+ $genecounter - 1,
      'index' => $genecounter,
...
// Print the resulting array
print_r($tempTopology);

//echo the genesInComplexes
echo "Genes in Complexes: " . json_encode($genesInComplexes) . "<br>";

//echo the genenamesInComplexes
// echo "Genenames in Complexes: " . json_encode($genenamesInComplexes) . "<br>";
...
foreach ($complexMap as $complexId => $c) {
    ...
    // Todo: populate the genesInComplex array from table tempMetadata instead of query
    ...
    for ($i = 0; $i < count($genesInComplexArray); $i++) {
        for ($j = 0; $j < count($genesInComplexArray); $j++) {
            if ($i != $j) {
                ...
                $src = $tempMetadata[$genesInComplexArray[$i]+$firstRightLabel]['region_name'];
                $trgt = $tempMetadata[$genesInComplexArray[$j]+$firstRightLabel]['region_name'];

                // split the source and target by underscore
                $source = explode("_", $src)[0];
                $target = explode("_", $trgt)[0];
                $sourceAssy = explode("_", $src)[1];
                $targetAssy = explode("_", $trgt)[1];

                // if the source and target are in the same complex and the source and target are in the pin table
                // if assypath is empty or the sourceAssy and targetAssy are in the assypath
                if ( ($pin[$source][$target]) &&
                    ($assypath == "" ||
                            (strpos($assypath, $sourceAssy) !== false &&
                            strpos($assypath, $targetAssy) !== false) &&
                            // sourceAssy and targetAssy are next to eachother in assypath
                            (strpos($assypath, $sourceAssy.$targetAssy) !== false ||
                            strpos($assypath, $targetAssy.$sourceAssy) !== false ))
                            ) {


                    //echo the source, target
                    if($debug) {
                        echo "Source: " . json_encode($source) . " Target: " . json_encode($target)   . "<br>";
                    }
...
$stmt = $pdo->prepare( 'INSERT INTO tempNetwork (source, target, interaction) VALUES ( :source, :target, :interaction )'  );
$stmt->bindValue(':source', $genesInComplexArray[$i-1], PDO::PARAM_STR);
$stmt->bindValue(':target', $genesInComplexArray[$j-1], PDO::PARAM_STR);
$stmt->bindValue(':interaction', 0, PDO::PARAM_STR);
$result = $stmt->execute();
if (!$result) {
  echo "Error inserting data: " . $pdo->errorInfo();
}
...
echo "Done!".$genesInComplexArray[$i-1].','.$source.','.$genesInComplexArray[$j-1].','.$target.','.$interaction;
?>
